...
def test_get_users():
    assert get_users() == [{"id": 1, "name": "Wojciech", "lastname": "Oczkowski"}, {"id": 69, "name": "Warol", "lastname": "Kojtyła"}]

def test_get_user_by_id():
    assert get_user_by_id(69) == {"id": 69, "name": "Warol", "lastname": "Kojtyła"}
...
def test_update_user(client):
    ...
    assert get_user_by_id(1) == {'id': 69, 'lastname': 'Kojtyła', 'name': 'Karol'}
...
def test_remove_user(client):
    ...
    assert get_users() == [{'id': 69, 'lastname': 'Kojtyła', 'name': 'Warol'}]

def test_remove_user_not_existing(client):
    client.delete("/users/5")
    assert get_users() == [{"id": 1, "name": "Wojciech", "lastname": "Oczkowski"}, {'id': 69, 'lastname': 'Kojtyła', 'name': 'Warol'}]